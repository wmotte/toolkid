% [ image, nii ] = procmsme( filename, outputPath, factorRO, factorPE )
%
function [ image, nii ] = procmsme( filename, outputPath, factorRO, factorPE ),
...
% order FID: RO x PE x echo x slices
fid = permute( fid, [ 1 3 2 ] );
fid = reshape( fid, numberOfRO, numberOfPE, numberOfEchos, numberOfSlices );
...
for slice = 1 : numberOfSlices,
  for echo = 1 : numberOfEchos, 
    trace = fid( :, :, echo, sliceIndex( slice ) );
    if mod( echo, 2 ) == 0,
      trace = flipdim( trace, 2 );
    end

    image( :, :, slice, echo ) = fft2d( trace, zeroFillRO, zeroFillPE );
  end
end
