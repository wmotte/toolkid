% [ image, nii ] = procmsepi( referencePath, inputPath, outputPath, factorRO, factorPE )
%
function [ image, nii ] = procmsepi( referencePath, inputPath, outputPath, factorRO, factorPE ),
...
numberOfShots = numberOfTraces / numberOfSlices; % number of shots
numberOfPE = numberOfPE * numberOfShots; % number of phase lines
numberOfBlocks = header.nblocks; % number of blocks

% zero-fill in PE direction
zeroFillPE = pow2( ceil( log2( numberOfPE * factorPE ) ) );

% prepare output
image = zeros( zeroFillRO, zeroFillPE, numberOfSlices, numberOfBlocks );

% reshape: RO x PE x slices x blocks
fid = reshape( fid, numberOfRO, numberOfPE / numberOfShots, numberOfSlices, numberOfShots, numberOfBlocks );
fid = permute( fid, [ 1 4 2 3 5 ] );
fid = reshape( fid, numberOfRO, numberOfPE, numberOfSlices, numberOfBlocks );
...
for block = 1 : numberOfBlocks, 
  for slice = 1 : numberOfSlices,
    ...
    for phase = 1 : numberOfPE,
      ...
      plane( :, phase ) = phasewithmap( trace, phasemap( :, phaseIndex, slice ) );
      if mod( phase, numberOfShots ) == 0,
