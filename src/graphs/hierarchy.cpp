...
protected:
	...
	void fit( const std::vector< PrecisionType >& clusteringCoefficients, const std::vector< PrecisionType > degrees, const std::string& csvFileName ) {
		...
		// init...
		vnl_sparse_matrix< PrecisionType > A( N, 2 );
		...
		for ( unsigned int i = 0; i < clusteringCoefficients.size(); i++ ) {
			...
			double x = clusteringCoefficients[i];
			double y = degrees[i];
		...
		std::cout << "Beta," << x[0] << std::endl;
		std::cout << "Beta_intercept," << x[1] << std::endl;
		std::cout << "Beta_residuals," << residuals << std::endl;
		...
		if ( csvFileName.size() != 0 ) {
			...
			if ( filestream.fail() ) {
				...
				filestream << "Beta," << x[0] << std::endl;
				filestream << "Beta_intercept," << x[1] << std::endl;
				filestream << "Beta_residuals," << residuals << std::endl << std::endl;
...
int main( int argc, char ** argv ) {
	tkd::CmdParser p( "graphhierarchy", "Calculate hierarchy parameter beta (weighted graphs only)" );
