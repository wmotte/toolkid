...
#include "initial_point.h"
	...
	static int mg_pdip_set( void *vstate, const gsl_cqp_data *cqp, gsl_vector *x, gsl_vector *y, gsl_vector *z, double *gap,
			double *residuals_norm, double *data_norm, double *infeasibility, double *infeasibility_min )
	{
		int status;
		size_t i, j, debug = 0;
	...
	static int mg_pdip_iterate( void *vstate, const gsl_cqp_data *cqp, gsl_vector *x, gsl_vector *y, gsl_vector *z, double *gap,
			double *residuals_norm, double *infeasibility, double *infeasibility_min )
	{
		unsigned int i, j;
		int status, signum;
		...
		for ( i = 0; i < z->size; i++ )
		{
			sigma += ( gsl_vector_get( z, i ) - alpha * gsl_vector_get( delta, cqp->Q->size2 + cqp->A->size1 + i ) ) * ( gsl_vector_get(
							state->s, i ) + alpha * gsl_vector_get( delta_s, i ) );
		}
		...
		for ( i = 0; i < delta_s->size; i++ )
		{
			...
			gsl_vector_set(
					delta_s,
					i,
					( -gsl_vector_get( r_zs, i ) + gsl_vector_get( delta, cqp->Q->size2 + cqp->A->size1 + i ) * gsl_vector_get( state->s, i ) )
					/ gsl_vector_get( z, i ) );
		}
		...
		while ( i < state->k_max )
		{
			...
			for ( j = 0; j < delta_s->size; j++ )
			{
				gsl_vector_set(
						delta_s_gondzio,
						j,
						( -gsl_vector_get( r_zs, j ) + gsl_vector_get( delta_gondzio, cqp->Q->size2 + cqp->A->size1 + j ) * gsl_vector_get(
										state->s, j ) ) / gsl_vector_get( z, j ) );
			}
	...
	static int compute_residuals( const gsl_cqp_data * cqp, const gsl_vector *x, const gsl_vector *y, const gsl_vector *z, const gsl_vector *s,
			gsl_vector *r )
	{
	...
	static int build_kkt_matrix( const gsl_cqp_data * cqp, const gsl_vector *z, const gsl_vector *s, gsl_matrix * kkt_matrix )
	{
		size_t i;
		...
		/* Null Block */
		kkt_block = gsl_matrix_submatrix( kkt_matrix, cqp->Q->size1, cqp->Q->size2, cqp->A->size1 + cqp->C->size1,
		...
		{
			gsl_matrix_set( kkt_matrix, i, i,
					-gsl_vector_get( s, i - ( cqp->Q->size1 + cqp->A->size1 ) ) / gsl_vector_get( z, i - ( cqp->Q->size1 + cqp->A->size1 ) ) );
		}
	...
	double step_length( const gsl_vector *s, const gsl_vector *z, const gsl_vector *delta_s, const gsl_vector *delta_z )
	{
		double alpha = 1.0;
		size_t i;
	...
	static double compute_gap_infeasible_points( const gsl_cqp_data *cqp, const gsl_vector *x, const gsl_vector *y, const gsl_vector *z )
	{
	...
	static void print_vectors( const gsl_vector * x, const gsl_vector * y, const gsl_vector * z, const gsl_vector * s )
	{
		unsigned int i;
	...
	{
			"mg_pdip", sizeof(mg_pdip_state), &mg_pdip_alloc, &mg_pdip_set, &mg_pdip_iterate, &mg_pdip_free
	};
