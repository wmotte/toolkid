...
int gsl_cqpminimizer_set( gsl_cqpminimizer * minimizer, gsl_cqp_data * cqp )
{
	...
	return ( minimizer->type->set )( minimizer->state, minimizer->cqp, minimizer->x, minimizer->y, minimizer->z, &( minimizer->gap ),
			&( minimizer->residuals_norm ), &( minimizer->data_norm ), &( minimizer->quantity_of_infeasibility ),
			&( minimizer->quantity_of_infeasibility_min ) );
}

void gsl_cqpminimizer_free( gsl_cqpminimizer * minimizer )
{
	( minimizer->type->free )( minimizer->state, minimizer->type->size );
	gsl_vector_free( minimizer->x );
	gsl_vector_free( minimizer->y );
	gsl_vector_free( minimizer->z );
...
int gsl_cqpminimizer_test_convergence( gsl_cqpminimizer * minimizer, double eps_gap, double eps_residuals )
{
...
int gsl_cqp_minimizer_test_infeasibility( gsl_cqpminimizer * minimizer, double eps_infeasible )
{
	...
	if ( minimizer->quantity_of_infeasibility > eps_infeasible && minimizer->quantity_of_infeasibility > pow( 10.0, 4 )
			* minimizer->quantity_of_infeasibility_min )
		return GSL_SUCCESS;
	else
		return GSL_CONTINUE;
