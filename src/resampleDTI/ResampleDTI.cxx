...
public:
	...
	{
		// TODO
		// set outputImageSpacing
		// set outputImageSize
		// set outputImageOrigin

		//verify if all the vector parameters have the good length
		if ( list.outputImageSpacing.size() != 3 || list.outputImageSize.size() != 3 || ( list.outputImageOrigin.size() != 3
				&& list.outputImageOrigin.size() != 0 ) || list.directionMatrix.size() != 9 || rotationPoint.size() != 3
		...
		parameters list;
		std::string inputVolume = "test.nii.gz"; // TODO
...
protected:
	...
	/**
	 * Verify if some input parametes are null.
	 */
	...
	/**
	 * Return pixeltype of image.
	 */
	void GetImageType( const std::string& fileName, itk::ImageIOBase::IOPixelType& pixelType,
			itk::ImageIOBase::IOComponentType& componentType )
	{
	...
	typename itk::DiffusionTensor3DInterpolateImageFunction< PixelType >::Pointer InterpolationType( std::string interpolationType,
			std::string windowFunction, unsigned int splineOrder )
	{
		...
		typedef itk::ConstantBoundaryCondition< itk::OrientedImage< PixelType, 3 > > BoundaryConditionType;
...
typedef		typename InterpolatorType::Pointer InterpolatorTypePointer;
	 ...
	 */
	template< class PixelType>
	typename itk::DiffusionTensor3DTransform< PixelType>::Pointer SetTransform( parameters list, typename itk::DiffusionTensor3DRead<
			PixelType>::Pointer &reader, itk::TransformFileReader::Pointer &transformFile )
	{
		...
		if( list.transformType.compare( "nr" ) ) //if rigid or affine transform
		{
			...
			{
				typename itk::DiffusionTensor3DRead< PixelType>::DiffusionImageType::IndexType index;
				typename itk::DiffusionTensor3DRead< PixelType>::DiffusionImageType::SizeType sizeim = reader->GetOutput()->GetLargestPossibleRegion().GetSize();
	 ...
	 */
	template< class PixelType>
	void SetOutputParameters( parameters list, typename itk::DiffusionTensor3DResample< PixelType, PixelType>::Pointer &resampler,
	...
	int Do( parameters list )
	{
		try
		{
			typedef itk::OrientedImage< itk::DiffusionTensor3D< PixelType>, 3> InputImageType;
			...
			interpol=InterpolationType< PixelType> ( list.interpolationType, list.windowFunction, list.splineOrder );
			...
			typedef typename TransformType::Pointer TransformTypePointer;
			// start transform
			// loop over the transform list (multiple transforms can be used through itkTransform files)
			do
			{
				TransformTypePointer transform;
		...
		{
			std::cerr << "Main: Do: Exception caught!" << std::endl;
...
{
	parameters list;
	list.outputImageSpacing = list.outputImageSpacing( 0 ); // use input image settings
	list.outputImageSize = list.outputImageSize( 0 ); // use input image settings
	list.outputImageOrigin = list.outputImageOrigin( 0 ); // use input image settings
	list.numberOfThread = 1;
	list.interpolationType = "bs";
	list.transformType = "";
	list.directionMatrix = list.directionMatrix( 0,0,0,0,0,0,0,0,0 );
	list.transformMatrix = list.transformMatrix( 1,0,0,0,1,0,0,0,1,0,0,0 );
	list.rotationPoint = list.rotationPoint( 0,0,0 );

	list.inputVolume;
	list.outputVolume;
	list.referenceVolume;
	list.transformationFile;

	list.inverseITKTransformation = false;
	list.windowFunction = "h";
	list.splineOrder = 3;

	list.space;
	list.centeredTransform = false;
	list.ppd = true;
	list.correction = "nearest";

	ResampleDTI resampleDTI;
	resampleDTI.Run( list );
	return EXIT_SUCCESS;
}
