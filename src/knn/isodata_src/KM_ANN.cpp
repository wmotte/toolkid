 ...
 */
KMdist kmDist( // interpoint squared distance, mahalanobis
		int dim, KMpoint p, KMpoint q, KMpoint sigma )
{
	register int d;
	register KMcoord diff;
	register KMcoord dist;
	...
	{
		diff = p[d] - q[d];
		dist = KM_SUM( dist, KM_POW( diff ) / KM_POW( sigma[d] ) );
	}
...
void kmCopyPt( // copy point w/o allocation
		int dim, const KMpoint source, KMpoint dest )
{
	for ( int i = 0; i < dim; i++ )
		dest[i] = source[i];
}
