...
static void postNeigh( // assign neighbors to center
		...
		KMpoint sumSqVec ); // the vector sum of squares, added by Nargess
	...
	bnd_box( dd ) // create initial bounding box
{
	...
	KMpoint ignoreMe2, ignoreMe4;
	double ignoreMe3;
	root->makeSums( ignoreMe1, ignoreMe2, ignoreMe3, ignoreMe4 );
		...
		KMpoint &theSumSqVec ) // vector sum of squares
{
	...
	{
		// visit low child
		child[i]->makeSums( n_child, s_child, ssq_child, ssqv_child );
		n_data += n_child; // increment no. points
		for ( int d = 0; d < kcDim; d++ )
		{ // update sum and sumSq
			sum[d] += s_child[d];
			sumSqVec[d] += ssqv_child[d];
		}
		sumSq += ssq_child;
	}
...
void KCleaf::makeSums( int &n, // number of points (returned)
		KMpoint &theSum, // sum (returned)
		double &theSumSq, // sum of squares (returned)
		KMpoint &theSumSqVec ) // the vector sum of squares
{
	assert( sum != NULL ); // should already be allocated
	assert( sumSqVec != NULL ); //added by Nargess
	...
	{ // compute sum
		for ( int d = 0; d < kcDim; d++ )
		{
			...
			sum[d] += theCoord;
			KMcoord theCoordSq = theCoord * theCoord;
			sumSqVec[d] += theCoordSq;
			sumSq += theCoordSq;
		}
	}
...
KCnode::~KCnode() // node destructor
{
	...
	if ( sumSqVec != NULL )
		kmDeallocPt( sumSqVec ); //deallocate sumSqVec
}
...
vector< KMpoint > kcCenters; // the center points
KMpointArray kcSums; // sums
KMpointArray kcSumSqVec; // array of sum of squares' vectors
double* kcSumSqs; // sum of squares
double* kcDists; // distortions
KMpoint kcBoxMidpt; // bounding-box midpoint
vector< vector< int > > clusters; // a vector of vectors, a vector for each center
// to contain indices of points in its neighborhood.

//----------------------------------------------------------------------
//  initDistGlobals - initialize distortion globals
//----------------------------------------------------------------------

// initialize distortion globals
static void initDistGlobals( KMfilterCenters& ctrs ) // the centers
{
	...
	kcSumSqVec = ctrs.getSumSqVec( false ); // Nargess: gets vector of sum of squares.
	kcSumSqs = ctrs.getSumSqs( false );
	kcDists = ctrs.getDists( false );
	kcBoxMidpt = kmAllocPt( kcDim );

	/*Nargess: clear previous clusters*/
	int old_size = clusters.size();
	for ( int i = 0; i < old_size; i++ )
		clusters[i].clear();
	clusters.clear();

	for ( int j = 0; j < kcKCtrs; j++ )
	{ // initialize sums
		kcWeights[j] = 0;
		kcSumSqs[j] = 0;
		...
		/* the following is added by Nargess: set up place holders for clusters to put */
		/* points in them later. 						      */

		vector< int > v;
		clusters.push_back( v ); //a vector for each cluster to hold the point indices belonging
		// to it.
...
)
{
	...
	{ // only one cand left?
		...
		postNeigh( this, sum, sumSq, n_data, cands[0], sumSqVec );
		/* the following is added by Nargess */
		//put the points in subtrees in 'clusters[theCand]' cluster
		child[KM_LO]->putInCluster( cands[0] );
		child[KM_HI]->putInCluster( cands[0] );
		/* end Nargess' additions */
	} else
	{
		...
		theCand = multCand; // no unique candidate
...
)
{
	...
	{ // only one cand left?
		// post points as neighbors
		postNeigh( this, sum, sumSq, n_data, cands[0], sumSqVec ); //Nargess: added last parameter

		/* start additions by Nargess */
		putInCluster( cands[0] );
		/* end additions by Nargess */
	...
	{ // find closest centers
		...
		{ // for each point in bucket
			...
			postNeigh( this, kcPoints[bkt[i]], sumSq, 1, cands[minK], sumSqVec );//Nargess: added
			// last parameter
			clusters[cands[minK]].push_back( bkt[i] ); //this line was added by Nargess
		} // for i
	} //else

}
//-------------------------------------------------------------------------------------
//Nargess: This function traverse the tree recursively to visit and collect all points in
//	   current node and its subtree	and put them in their associated center's cluster.
//	   The index of the cluster to which points belong to is passed as parameter.
//-------------------------------------------------------------------------------------
void KCsplit::putInCluster( int c )
{
	child[KM_LO]->putInCluster( c );
	child[KM_HI]->putInCluster( c );

}
//-------------------------------------------------------------------------------------
//Nargess: Puts the each point in the node in its associated center's cluster.
//-------------------------------------------------------------------------------------
void KCleaf::putInCluster( int c )
{
	for ( int i = 0; i < n_data; i++ )
	{
		clusters[c].push_back( bkt[i] );
...
//	only if tracing.
//      Nargess: added last parameter to enable calculating vector of sum
//		 of squares.
//----------------------------------------------------------------------
...
static void postNeigh( KCptr p, // the node posting
		...
		int n_data, // number of points
		KMctrIdx ctrIdx, // center index
		KMpoint sumSqVec ) // Nargess: vector of sum of squares
{
	for ( int d = 0; d < kcDim; d++ )
	{ // increment sum
		kcSums[ctrIdx][d] += sum[d];
		kcSumSqVec[ctrIdx][d] += sumSqVec[d];
	}
