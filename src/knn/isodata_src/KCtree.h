...
//				with itself).
//		sumSqVec	This is a vector (point) whose coordinates
//				are sum of squares of each points' 
//				corresponding coordinates. 
//		bnd_box		Bounding box for the cell.
//		theCand		(Used for performance evaluation.)
//				If there is only one candidate, this is
//				set to the candidate's index, and otherwise
//				it is set to the special value, multCand = -1.
//
...
protected:
    ...
    KMpoint		sumSqVec;	// vector sum of squares		
    KMorthRect		bnd_box;	// bounding box for cell
    int			theCand;	// candidate (or multCand if many)
...
public:
    ...
    {  sum = kmAllocPt(dim, 0); 
       sumSq = 0; 
       theCand = multCand;  
       sumSqVec=kmAllocPt(dim,0); }     //Nargess added this line
	...
	KMpoint		&theSumSqVec) = 0;      // added by Nargess:sum of squares vector(returned) 
	...
	int		kCands)=0; 		// number of centers				
     ...
     virtual void putInCluster(int c)=0;	//Nargess: Puts all the points in the current cell
						//in cluster 'c' (passed parameter) 
...
public:
	...
	KMpoint		&theSum,		// sum (returned)
	double		&theSumSq,		// sum of squares (returned)
	KMpoint		&theSumSqVec);         // added by Nargess:sum of squares vector(returned) 
    ...
    virtual void putInCluster(int c);
					// sample a center point c
