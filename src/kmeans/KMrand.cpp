...
#ifdef _BORLAND_C
inline void srandom(int x)              // Borland C++ doesn't have srandom
  { srand(x); }
inline long random()                    // Borland C++ doesn't have random
  { return _lrand(); }
#endif
...
#elif USING_SUNOS55 			// SunOS 5.5 or earlier
	...
	double		&clus_sep)	// cluster separation (returned)
{
	...
	for (int j = i+1; j < n_col; j++) {
	    ...
	    if (dist < minDist) minDist = dist;
					...
					// cluster separation
    clus_sep = sqrt(minDist)/(sqrt(dim)*std_dev);
...
/**********************************************************************************/
/* This version of kmClusGaussPts generates Guassian clusters with different      */
/* standard deviations.  The standard deviation for each cluster is also randomly */
/* generated from the Guassian distribution.  The user passes the standard        */
/* deviation and mean of the distribution from which standard deviations are      */
/* generated (std_std, std_m)                                                     */
/* ********************************************************************************/ 
static KMpointArray STDVs = NULL;	// cluster storage
void kmClusGaussPts2(		        // clustered-Gaussian distribution
	...
	int		n_col,		// number of colors
	bool		new_clust,	// generate new clusters.
	double          std_m,           // standard deviation mean.
	double		std_std	         // standard deviation for clusters' standard deviation
	)
{
    if (cgClusters == NULL || new_clust) {// need new cluster centers
	if (cgClusters != NULL)		// clusters already exist
    ...
    if (STDVs == NULL || new_clust) {// need new standard deviation values for clusters
	if (STDVs != NULL)		// clusters already exist
	    kmDeallocPts(STDVs);	// get rid of them
	STDVs = kmAllocPts(n_col, 1);
					// generate cluster center coords

	double offset= sqrt(3)*std_std;
	for (int i = 0; i < n_col; i++) 
	  {
	    // We select the interval in such a way that if we randomly slect a std
	    // from that interval using a Guassian distribution, then they would have
	    // the user specified std_std and std_m.
		STDVs[0][i]= (KMcoord) kmRanUnif(std_m - offset , std_m + offset);
	...
	int c = kmRanInt(n_col);		// generate cluster index
	for (int d = 0; d < dim; d++) {
          pa[i][d] = (KMcoord) (STDVs[0][c]*kmRanGauss() + cgClusters[c][d]);
	}
    }
}
/*********************************************************************/
