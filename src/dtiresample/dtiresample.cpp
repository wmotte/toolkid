...
#define RADIUS 3
	...
	typename itk::DiffusionTensor3DInterpolateImageFunction< PixelType >::Pointer InterpolationTypeFct( std::string interpolationType,
			std::string windowFunction, unsigned int splineOrder, int numberOfThreads )
	{
		...
		typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction< PixelType, RADIUS, HammingwindowFunctionType,
		...
		typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction< PixelType, RADIUS, CosinewindowFunctionType,
		...
		typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction< PixelType, RADIUS, WelchwindowFunctionType,
		...
		typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction< PixelType, RADIUS, LanczoswindowFunctionType,
		...
		typedef itk::DiffusionTensor3DWindowedSincInterpolateImageFunction< PixelType, RADIUS, BlackmanwindowFunctionType,
	 ...
	 */
	template< class PixelType >
	itk::Matrix< double, 4, 4 > ComputeTransformMatrix( const parameters &list, const typename itk::Image< itk::DiffusionTensor3D<
			PixelType >, 3 >::Pointer &image, const itk::Point< double > &outputImageCenter )
	{
	...
	typename itk::DiffusionTensor3DAffineTransform< PixelType >::Pointer FSOrPPD( const std::string &ppd,
			itk::Matrix< double, 4, 4 > *matrix = NULL )
	{
	 ...
	 */
	template< class PixelType >
	typename itk::DiffusionTensor3DTransform< PixelType >::Pointer SetUpTransform( const parameters &list, const typename itk::Image<
			itk::DiffusionTensor3D< PixelType >, 3 >::Pointer &image,
			const typename itk::DiffusionTensor3DNonRigidTransform< PixelType >::TransformType::Pointer &nonRigidFile,
			bool precisionChecking, const itk::Point< double > & outputImageCenter )
	{
	 ...
	 */
	template< class PixelType >
	typename itk::DiffusionTensor3DTransform< PixelType >::Pointer SetTransformAndOrder( parameters &list, const typename itk::Image<
			itk::DiffusionTensor3D< PixelType >, 3 >::Pointer &image,
			...
			const itk::Point< double > &outputImageCenter )
	{
		...
		if ( list.transformationFile.compare( "" ) )
		{
			...
			typename itk::MatrixOffsetTransformBase< double, 3, 3 >::Pointer doubleMatrixOffsetTransform;
			typename itk::MatrixOffsetTransformBase< float, 3, 3 >::Pointer floatMatrixOffsetTransform;
			typename FSAffineTransformType::Superclass::AffineTransformType::Pointer doubleAffineTransform =
					dynamic_cast< typename FSAffineTransformType::Superclass::AffineTransformType* > ( transform.GetPointer() );
			...
			} else
			{
				...
				{
					typename RigidTransformType::Rigid3DTransformType::Pointer doubleRigid3DTransform =
							dynamic_cast< typename RigidTransformType::Rigid3DTransformType* > ( transform.GetPointer() );
					...
					} else
					{
						...
						{
							nonRigidFile = dynamic_cast< typename NonRigidTransformType::TransformType* > ( transform.GetPointer() );
	 ...
	 */
	template< class PixelType >
	typename itk::DiffusionTensor3DTransform< PixelType >::Pointer SetTransform( parameters &list, const typename itk::Image<
			itk::DiffusionTensor3D< PixelType >, 3 >::Pointer &image, itk::TransformFileReader::Pointer &transformFile, const itk::Point<
			double > &outputImageCenter )
	{
		...
		if ( list.transformationFile.compare( "" ) )
		{
			...
			{
				transform = dynamic_cast< TransformType* > ( transformFile->GetTransformList()->back().GetPointer() );
			} else
			{
	...
	If the transform file contain a transform that the program does not handle, the function returns -1.
	 */
	template< class PixelType >
	int ReadTransform( parameters &list, const typename itk::Image< itk::DiffusionTensor3D< PixelType >, 3 >::Pointer &image,
	 ...
	 */
	template< class PixelType >
	void SetOutputParameters( const parameters &list, typename itk::DiffusionTensor3DResample< PixelType, PixelType >::Pointer &resampler,
	 ...
	 */
	void ResampleDeformationField( DeformationImageType::Pointer &field, const itk::Point< double, 3 > &origin, const itk::Vector< double,
			3 > &spacing, const itk::Size< 3 > &size, const itk::Matrix< double, 3, 3 > &direction )
	{
	 ...
	 */
	template< class PixelType > int Do( parameters list )
	{
				...
				|| list.deffield.compare( "" ) )
		{
			...
			if ( list.deffield.compare( "" ) )
			{
				...
				ResampleDeformationField( field, dummyOutputImage->GetOrigin(), dummyOutputImage->GetSpacing(),
						dummyOutputImage->GetLargestPossibleRegion().GetSize(), dummyOutputImage->GetDirection() );
			...
			while ( list.transformationFile.compare( "" ) && transformFile->GetTransformList()->size() )
			{
				...
				if ( !list.notbulk && transform->GetTransform()->GetTransformTypeAsString() == "BSplineDeformableTransform_double_3_3"
						&& transformFile->GetTransformList()->size() )//Check if transform file contains a BSpline
				{
					//transform = SetTransform< PixelType > ( list , image , transformFile , outputImageCenter ) ;
...
int main( int argc, char * argv[] )
{
	...
	std::string inputVolume = "test.nii.gz";
	...
	// TODO overnamen transform van pss0.cpp (spatialix code) en ResampleDTIFilter imp.
}
