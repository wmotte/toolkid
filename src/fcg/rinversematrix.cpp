...
template< class T >
T r_to_z( const T& r, const int& n )
{
  return vcl_log( ( 1. + r ) / ( 1. - r ) ) / ( 2. * vcl_sqrt( 1. / ( static_cast< T >( n - 3 ) ) ) );
}

void RMatrix::Run( const std::string& inputFileName, const std::string& maskFileName, const std::string& outputFileName )
{
  ...
  MatrixType matrix = source.extract( source.rows() - 100, source.cols(), 50, 0 );
...
int main( int argc, char ** argv )
{
  if ( argc != 4 )
    {
    std::cout << "Usage: " << argv[ 0 ] << " [input 4D image] [input 3D mask] [output 2D image]" << std::endl;
    return -1;
    }

  fcg::RMatrix rMatrix;
  rMatrix.Run( argv[ 1 ], argv[ 2 ], argv[ 3 ] );
