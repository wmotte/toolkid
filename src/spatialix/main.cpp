...
int main( int argc, char ** argv )
{
	...
	std::string outputMovingXFileName;
	std::string outputMovingYFileName;
	std::string outputMovingZFileName;

	PixelType defaultPixelValue = 0;
	bool noMatrixComposition = false;

	p.AddArgument( transformFileNames, "transforms" ) ->AddAlias( "t" ) ->SetDescription(
			"Input transforms (supported: SimilarityTransform, AffineTransform, BSplineTransform).\n        Prefix:\n        +i = Invert transformation\n        +s = Skip reading initial transformation\n        +a = Combine by addition (default: composition)\n        +n = Normalize B-spline transform (if combined by addition)" );

	p.AddArgument( referenceFileName, "reference" ) ->AddAlias( "r" ) ->SetDescription( "Reference image" );

	p.AddArgument( inputFileName, "input" ) ->AddAlias( "i" ) ->SetDescription( "Input image to resample" );

	p.AddArgument( outputImageFileName, "output-image" ) ->AddAlias( "o" ) ->SetDescription( "Write resampled image" );

	p.AddArgument( outputTransformFileName, "output-transform" ) ->AddAlias( "ot" ) ->SetDescription( "Output transform path" );

	p.AddArgument( outputJDFileName, "jacobian-determinant" ) ->AddAlias( "jd" ) ->SetDescription( "Write Jacobian determinant" );

	p.AddArgument( outputDeformationFieldFileName, "deformation-field" ) ->AddAlias( "df" ) ->SetDescription( "Write deformation field" );

	p.AddArgument( outputMovingXFileName, "output-moving-x-coordinates" ) ->AddAlias( "omx" ) ->SetDescription( "Write physical moving x coordinates" );

	p.AddArgument( outputMovingYFileName, "output-moving-y-coordinates" ) ->AddAlias( "omy" ) ->SetDescription( "Write physical moving y coordinates" );

	p.AddArgument( outputMovingZFileName, "output-moving-z-coordinates" ) ->AddAlias( "omz" ) ->SetDescription( "Write physical moving z coordinates" );



	p.AddArgument( interpolationOrder, "order" ) ->AddAlias( "d" ) ->SetDescription( "Interpolation order" );

	p.AddArgument( defaultPixelValue, "default-pixel" ) ->AddAlias( "dp" ) ->SetDescription(
			"Default pixel value when resampling (default: 0)" );

	p.AddArgument( noMatrixComposition, "no-matrix-composition" ) ->AddAlias( "n" ) ->SetDescription(
			"Disable matrix composition of matrix transforms" );
	...
	{
		itk::ImageIOBase::Pointer io = itk::ImageIOFactory::CreateImageIO( inputFileName.c_str(), itk::ImageIOFactory::ReadMode );
	...
	if ( outputMovingXFileName != "" || outputMovingYFileName != "" || outputMovingZFileName != "" )
	{
		ImageType::RegionType region = reference->GetLargestPossibleRegion();

		ImageType::Pointer movingXImage = ImageType::New();
		ImageType::Pointer movingYImage = ImageType::New();
		ImageType::Pointer movingZImage = ImageType::New();

		movingXImage->CopyInformation( reference );
		movingXImage->SetRegions( region );
		movingXImage->Allocate();
		movingXImage->FillBuffer( 0 );
		movingYImage->CopyInformation( reference );
		movingYImage->SetRegions( region );
		movingYImage->Allocate();
		movingYImage->FillBuffer( 0 );
		movingZImage->CopyInformation( reference );
		movingZImage->SetRegions( region );
		movingZImage->Allocate();
		movingZImage->FillBuffer( 0 );

		itk::ImageRegionIteratorWithIndex< ImageType > itMovingX( movingXImage, region );
		itk::ImageRegionIteratorWithIndex< ImageType > itMovingY( movingYImage, region );
		itk::ImageRegionIteratorWithIndex< ImageType > itMovingZ( movingZImage, region );
		...
		for ( ; !itMovingX.IsAtEnd(); ++itMovingX, ++itMovingY, ++itMovingZ )
		{
			movingXImage->TransformIndexToPhysicalPoint( itMovingX.GetIndex(), inputPoint );
			outputPoint = t->TransformPoint( inputPoint );

			itMovingX.Set( outputPoint[0] );
			itMovingY.Set( outputPoint[1] );
			itMovingZ.Set( outputPoint[2] );
		}

		if ( !outputMovingXFileName.empty() )
		{
			WriterType::Pointer writer = WriterType::New();
			writer->SetInput( movingXImage );
			writer->SetFileName( outputMovingXFileName );
			writer->Update();
		}

		if ( !outputMovingYFileName.empty() )
		{
			WriterType::Pointer writer = WriterType::New();
			writer->SetInput( movingYImage );
			writer->SetFileName( outputMovingYFileName );
			writer->Update();
		}

		if ( !outputMovingZFileName.empty() )
		{
			WriterType::Pointer writer = WriterType::New();
			writer->SetInput( movingZImage );
			writer->SetFileName( outputMovingZFileName );
			writer->Update();
		}
